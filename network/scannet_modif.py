...
def neighborhood_embedding(
    ...
    frames = neighborhoods.FrameBuilder(name='frames_%s' % scale,
                                        ...
                                        dipole=dipole_frame
                                        # Only used if using a quadruplet of indices. Then, frame has a 4rth additional direction (the dipole).
    ...
    if 'index_distance' in coordinates:  # Add sequence position distance.
        ...
        if isinstance(attributes, list):  # list if we add vectors
    ...
    embedded_local_coordinates = embeddings.GaussianKernel(Ngaussians,
                                                           ...
                                                           name='embedded_local_coordinates_%s' % scale)(
        local_coordinates)
    ...
    if nrotations > 1:
        ...
        spatiochemical_filters_activity = utils.MaxAbsPooling(axis=-2, name='SCAN_filter_activity_%s' % scale)(
            spatiochemical_filters_input)
    else:
        spatiochemical_filters_activity = addNonLinearity(spatiochemical_filters_input, activation,
                                                          name='SCAN_filter_activity_%s' % scale)  # Add non-linearity.
    if vectors:
        if return_frames:
            output =  spatiochemical_filters_activity, frames, graph_aligned_vectors
        else:
            output = spatiochemical_filters_activity, frames
    else:
        if return_frames:
            output = spatiochemical_filters_activity, frames
        else:
            output = spatiochemical_filters_activity
    return output
...
def ScanNet(
    ...
    frame_indices_aa = Input(shape=[Lmax_aa, nindex], name='frame_indices_aa',
    ...
    point_clouds_aa = Input(shape=[Lmax_aa_points, 3], name='point_clouds_aa',
                            dtype="float32")  # 3D coordinates of the amino acids point clouds. (Calpha coordinates, side chain center of mass,...).
    ...
    if with_atom:
        ...
        masked_sequence_indices_atom = Masking(mask_value=-1, name='masked_sequence_indices_atom')(
    ...
    if nembedding_aa is not None:  # Apply point-wise dense embedding.
        embedded_attributes_aa = attribute_embedding(masked_attributes_aa, nembedding_aa, activation,
                                                     name='embedded_attributes_aa')
    else:  # Normalize such that that variance is approximately one.
        normalizer = np.sqrt(nfeatures_aa)
        embedded_attributes_aa = TimeDistributed(Lambda(lambda x: normalizer * x), name='embedded_attributes_aa')(
            masked_attributes_aa)
    ...
    if with_atom:
        ...
        # Here pooling_mask is a binary matrix of size [L_aa, L_atom], with M_{ij} =0 if atom j belongs to amino acid i.
        ...
        SCAN_filters_atom_aggregated_activity = addNonLinearity(SCAN_filters_atom_aggregated_input, activation,
                                                                name='SCAN_filters_atom_aggregated_activity')
        # Add final non-linearity.

    if with_atom:
        all_embedded_attributes_aa = Concatenate(name='all_embedded_attributes_aa', axis=-1)(
            [embedded_attributes_aa, SCAN_filters_atom_aggregated_activity])
    else:
    ...
    if motion_vectors:
        SCAN_filters_aa, frames_aa, graph_aligned_vectors = neighborhood_embedding(
            ...
            [all_embedded_attributes_aa, masked_attributes_motion_aa],
            vectors=True,  # we notify existing motion vectors
        ...
        SCAN_filters_aa, frames_aa = neighborhood_embedding(
            ...
            vectors=False,  # we notify existing motion vectors
            sequence_indices=masked_sequence_indices_aa,
            order_frame=order_aa,
            dipole_frame=dipole_aa,
            Kmax=K_aa,
            ...
            return_frames=True)
    """
    # Eigenvectors sign-invariance treatment
    if with_motion_vectors:
        SignNet = signnet.GINDeepSigns(6, 6, 6, 1, m)
...
def initialize_ScanNet(
        ...
        dropout=0.,
        optimizer='adam',
        initial_values_folder='models/initial_values/',
    ...
    initial_values = {'GaussianKernel_aa': None, 'GaussianKernel_atom': None, 'GaussianKernel_graph': None,
                      'dense_graph': None}
