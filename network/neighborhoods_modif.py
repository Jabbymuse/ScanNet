...
class LocalNeighborhood(Layer):
    ...
    def call(self, inputs,mask=None,vectors=False):
        ...
        if vectors:
            second_attributes,attributes_vectors = inputs[-self.nattributes:-1],[inputs[-1]] # list for neighbor gathering
        else:
        ...
        if vectors:
            neighbors_motion_attributes = [tf.gather_nd(
                attribute, neighbors, batch_dims=1) for attribute in attributes_vectors] # B * 256 * K * m * 6 (graph)
        ...
        if 'euclidian' in self.coordinates:
            ...
            if vectors :
                m = neighbors_motion_attributes[0].shape[3] # number of motions

                Batch_rot = tf.expand_dims(tf.tile(tf.expand_dims(first_frame[:, :, 1:4], axis=-3), multiples=[1, 1, m, 1, 1]),axis=-4)  # B X Lmax X 1 X m X 3 X 3


                aligned_v = tf.expand_dims(neighbors_motion_attributes[0][:,:,:,:,:3],axis=-1) # B X Lmax X Kmax X m X 3 X 1
                aligned_v = tf.reduce_sum(aligned_v*Batch_rot,axis=-2) # B X Lmax X Kmax X m X 3

                aligned_w = tf.expand_dims(neighbors_motion_attributes[0][:, :, :, :, 3:],axis=-1)  # B X Lmax X Kmax X m X 3 X 1
                aligned_w = tf.reduce_sum(aligned_w*Batch_rot, axis=-2)  # B X Lmax X Kmax X m X 3


                graph_aligned_vectors = tf.concat([aligned_v,aligned_w],axis=-1) # B X Lmax X Kmax X m X 6
        ...
        if vectors:
            return output + [graph_aligned_vectors]
        else:
            return output
