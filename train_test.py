...
def make_PR_curves(
    ...
    for i in range(nSubsets):
        ...
        weights_repeated = np.array([np.ones(len(label)) * weight for label, weight in zip(labels, weights)],
                                    dtype=np.object)
...
if __name__ == '__main__':
    ...
    list_dataset_locations = ['datasets/PPBS/tests_labels_%s.txt'% dataset for dataset in list_datasets]
    ...
    Input format:
    [
    input[0]: amino acid-wise triplets of indices for constructing frames ([Naa,3], integer). difference with point cloud ??
    input[1]: amino acid-wise attributes: either one-hot-encoded sequence or position-weight matrix ([Naa,20/21],). Which Attributes ??
    ...
    if train:
        ...
        validation_inputs = [np.concatenate([list_inputs[i][j] for i in [1] ] ) for j in range( len(list_inputs[0]) ) ]
        validation_outputs = np.concatenate([list_outputs[i] for i in [1]])
        validation_weights = np.concatenate([list_weights[i] for i in [1]])
        ...
        model, extra_params = scannet.initialize_ScanNet(
            ...
            Dmax_aa=11.,
            # Cut-off distance for the amino acid NEM. Only used when initializing the aa gaussian kernels.
            Dmax_atom=4.,  # Cut-off distance for the atom NEM. Only used when initializing the gaussian kernels.
            Dmax_graph=13.,
            # Cut-off distance for the amino acid NAM. Only used when initializing the gaussian kernels.
            ...
            nembedding_atom=12,
            # Dimension of atom attribute embedding. If = nfeatures_atom, use non-trainable one-hot-encoding.
            ...
            filter_MLP=[32],
            # Dimensionality reduction (trainable dense layer) applied after amino acid NEM and before NAM.
    ...
    test_predictions = [
        ...
        for i in [2] # 5--8 for tests
    ]

    test_labels = [np.array([
        np.argmax(label, axis=1)[:Lmax_aa]  # Back to 0-1 labels from one-hot encoding and truncate to Lmax.
        for label in list_outputs[i]]) for i in [2]]

    test_weights = [list_weights[i] for i in [2]]
