import os
from preprocessing.PDB_processing import create_reduce_pdb,NOLB_to_motion_vectors
from network.scannet_modif import initialize_ScanNet
...
from preprocessing import pipelines_modifie as pipelines, PDB_processing, sequence_utils, PDBio
from utilities import wrappers, chimera
import numpy as np
import argparse
from keras.models import Model
from utilities.paths import structures_folder, MSA_folder, predictions_folder, path2hhblits, path2sequence_database, \
    model_folder
...
def predict_interface_residues(
        ...
        while i < npdbs:
            try:
                ...
                if query_chain_ids[i] == 'all':
                    ...
                    query_chain_ids[i] = [(chain_obj.get_full_id()[1], chain_obj.get_full_id()[2])
                                          for chain_obj in chain_objs if
                                          (chain_obj.get_full_id()[2].isupper() | (chain_obj.get_full_id()[2] == ' '))]
                ...
                while j < nsequences:
                    ...
                    if len(sequence) < Lmin:
                        print('Chain %s %s from PDB %s is too short (L=%s), discarding.' % (
                            query_chain_ids[i][j][0], query_chain_ids[i][j][1], query_pdbs[i], len(sequence)),
                              file=logfile)
                    ...
                    if not (os.path.exists(target_location) & (~overwrite_MSA)):
                        if sequence in query_sequences[i][:j]:
                            ...
                            sequence_utils.call_hhblits(sequence, target_location,
                                                        ...
                                                        path2sequence_database=path2sequence_database, cores=cores)
    ...
    if isinstance(model, list):
        ...
        multi_models = False
        model_obj = wrappers.load_model(model_folder + model, Lmax=Lmax, with_vectors=True) # old_model
        model_objs = None
    ...
    if layer is not None:
        if isinstance(layer, list):
            ...
            if layer == 'attention_layer':
                model_obj.model = Model(inputs=model_obj.model.inputs,
                                        outputs=model_obj.model.get_layer(layer).output[1])
            else:
    ...
    if assembly:
        inputs = wrappers.stack_list_of_arrays(
            [pipeline.process_example(chain_obj=chain_obj, sequence=sequence, MSA_file=MSA_file_location, PWM=PWM)[0]
             for chain_obj, sequence, MSA_file_location, PWM in
             zip(query_chain_objs, query_sequences, query_MSAs, query_PWMs)], padded=padded)
        if multi_models:
            if aggregate_models:
                ...
                for model_obj in model_objs[1:]:
                    ...
                    query_predictions = [prediction1 + prediction2 for prediction1, prediction2 in
                                         zip(query_predictions, predictions)]
                query_predictions = np.array([prediction / len(model_objs) for prediction in query_predictions])
            else:
                query_predictions = [model_obj.predict(inputs, batch_size=1, return_all=return_all) for model_obj in
                                     model_objs]
        else:
        ...
        if has_attention_layer:
            ...
            '''
            calpha_coordinates = [inputs[3][n].astype(np.float32)[inputs[0][n].astype(np.int)[..., 0]] for n in
                                  range(len(inputs[0]))]
            K_graph = model_obj.kwargs['K_graph']
            neighborhood_graphs = [
                np.argsort(PDB_processing.distance(calpha_coordinate, calpha_coordinate), axis=1)[:, :K_graph] for
                calpha_coordinate in calpha_coordinates]
            if layer == 'attention_layer':
            ...
            for attention_coeff, neighborhood_graph in zip(attention_coeffs, neighborhood_graphs):
                ...
                for s in range(len(attention_coeff)):
                    aggregated_attention_coeff[neighborhood_graph[s]] += np.maximum(
                        sign * attention_coeff[s][:len(neighborhood_graph[s])], 0).mean(
        ...
        query_predictions = []
        for i in range(nqueries):
            # computation of motion_vectors
            chain_objs = query_chain_objs[i]
            # creation of file paths
            pdb_name = chain_objs[0].full_id[0]
            input_pdb_file = f"PDB/pdb{pdb_name}.bioent"
            inter_H_pdb_file = f"PDB/pdb{pdb_name}_H_reduced.bioent"
            inter_pdb_file = f"PDB/pdb{pdb_name}_pre_reduced.bioent"
            output_pdb_file = f"PDB/pdb{pdb_name}_reduced.bioent"
            NOLB_command = f"motion/NOLB {output_pdb_file} -f 2"
            create_reduce_pdb(input_pdb_file,inter_H_pdb_file,inter_pdb_file,output_pdb_file,chain_objs,NOLB_command) # creation of the motion_vectors file
            aa_indices_lengths = sequence_lengths[0]
            pdb_name = chain_objs[0].full_id[0]
            motion_vectors_path = f"PDB/pdb{pdb_name}_reduced_rtb.txt"
            list_motion_vectors = NOLB_to_motion_vectors(aa_indices_lengths,motion_vectors_path,m)
            inputs = wrappers.stack_list_of_arrays(
                [pipeline.process_example(chain_obj=chain_obj, sequence=sequence, MSA_file=MSA_file_location, PWM=PWM, motion_vectors=np.array(motion_vectors))[
                     0]
                 for chain_obj, sequence, MSA_file_location, PWM, motion_vectors in
                 zip(query_chain_objs[i], query_sequences[i], query_MSAs[i], query_PWMs[i], list_motion_vectors)], padded=padded)

            # creation of the new_model
            new_model = initialize_ScanNet(inputs,None,with_motion_vectors=True,index_distance_max_graph=8)
            ...
            if multi_models:
                if aggregate_models:
                    ...
                    for model_obj in model_objs[1:]:
                        ...
                        predictions = [prediction1 + prediction2 for prediction1, prediction2 in
                                       zip(predictions, predictions_)]
                    predictions = np.array([prediction / len(model_objs) for prediction in predictions])
                else:
                    predictions = [model_obj.predict(inputs, batch_size=1, return_all=return_all) for model_obj in
                                   model_objs]
            else:
                predictions = model_obj.predict(inputs, batch_size=1, return_all=return_all)
        ...
        if ((isinstance(layer, list)) | (isinstance(layer, tuple)) | (not aggregate_models)):
            query_predictions = [[query_predictions[k][l] for k in range(len(query_predictions))] for l in
                                 range(len(query_predictions[0]))]
            ...
            if ((isinstance(layer, list)) | (isinstance(layer, tuple)) | (not aggregate_models)):
                for layer_, prediction in zip(layer, predictions):
                    if layer_ is None:
                        ...
                        annotated_pdb_file = query_output_folder + 'annotated_' + query_names[i] + (
                            '.cif' if file_is_cif else '.pdb')
                    else:
                        ...
                        annotated_pdb_file = query_output_folder + 'annotated_%s' % layer_ + query_names[i] + (
                            '.cif' if file_is_cif else '.pdb')
                    ...
                    if predict_from_pdb & (prediction.ndim == 1):
                        if output_chimera == 'script':
                            chimera.show_binding_sites(
                                query_pdbs[i], csv_file, chimera_file, biounit=biounit, directory='',
                            ...
                            chimera.annotate_pdb_file(pdb_file_locations[i], csv_file, annotated_pdb_file,
                                                      output_script=True, mini=mini, maxi=maxi,
                                                      version='surface' if assembly else 'default')
            else:
                if layer is None:
                    csv_file = query_output_folder + 'predictions_' + query_name + '.csv'
                    chimera_file = query_output_folder + 'chimera_' + query_names[i]
                    ...
                    annotated_pdb_file = query_output_folder + 'annotated_%s' % layer + query_names[i] + (
                        '.cif' if file_is_cif else '.pdb')
    ...
    if output_format == 'dictionary':
        if ((isinstance(layer, list)) | (isinstance(layer, tuple)) | (not aggregate_models)):
            query_dictionary_predictions = [PDB_processing.make_values_dictionary(query_residue_ids[k],
                                                                                  [query_predictions[l][k] for l in
                                                                                   range(len(query_predictions))])
                                            for k in range(len(query_residue_ids))]
        else:
            query_dictionary_predictions = [PDB_processing.make_values_dictionary(query_residue_id, query_prediction)
                                            for query_residue_id, query_prediction in
                                            zip(query_residue_ids, query_predictions)]
        return query_pdbs, query_names, query_dictionary_predictions
    else:
        if ((isinstance(layer, list)) | (isinstance(layer, tuple)) | (not aggregate_models)):
            query_predictions = [
                [query_predictions[i][j] for i in range(len(query_predictions))] for j in
                range(len(query_predictions[0]))]
